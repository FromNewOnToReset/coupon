...
import org.sllxgmm.coupon.converter.CouponCategoryConverter;
import org.sllxgmm.coupon.converter.DistributeTargetConverter;
import org.sllxgmm.coupon.converter.ProductLineConverter;
import org.sllxgmm.coupon.converter.RuleConverter;
import org.sllxgmm.coupon.serialization.CouponTemplateSerialize;
...
public class CouponTemplate {
    ...
    public CouponTemplate(String name, String logo, String desc, String category,
        ...
        this.logo = logo;
        this.desc = desc;
        this.category = CouponCategory.ofValue(category);
        this.productLine = ProductLine.ofValue(productLine);
        this.count = count;
        this.userId = userId;
        // 优惠券模板唯一编码 = 4(产品线和类型) + 8(日期: 20190101) + id(扩充为4位)
        this.key = productLine.toString() + category +
                new SimpleDateFormat("yyyyMMdd").format(new Date());
        this.target = DistributeTarget.ofValue(target);
        this.rule = rule;
    }
