...
import org.sllxgmm.coupon.vo.GoodsInfo;
...
public abstract class AbstractExecutor {
    ...
    protected SettlementInfo processGoodsTypeNotSatisfy(
            SettlementInfo settlement, BigDecimal goodsSum
    ) {
    ...
    protected BigDecimal goodsCostSum(List<GoodsInfo> goodsInfos) {
        ...
        for (int i = 0; i <goodsInfos.size() ; i++) {
            result.add(goodsInfos.get(i).getDecimalPrice().multiply(new BigDecimal(goodsInfos.get(i).getCount())));
        }
        return result;
    }
     ...
     * <h2>保留两位小数</h2>
     * */
    protected double retain2Decimals(BigDecimal value) {
